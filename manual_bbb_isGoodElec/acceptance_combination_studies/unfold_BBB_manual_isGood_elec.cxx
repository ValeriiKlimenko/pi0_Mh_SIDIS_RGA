...
static bool build_histograms(HistBundle& out)
{
  ...
  for (int i=1;i<=20;++i) {
    std::string f_rt = Form(
      ...
      "unfolding_rec_true/h3_bin_xBQ2_Valerii_%d_fitted.root", i);
  ...
  {
    const char* miss2d_file  = "/w/hallb-scshelf2102/clas12/valerii/multiPi0/pass2_v3/gen_binning_2D_isGoodElec_no_gemc512.root";
...
// -----------------------------------------------------------------------------
static int make_scaled_hist(const char* out_file = "measData_times_truth_over_meas_iGoodElec.root")
{
  HistBundle pack;
