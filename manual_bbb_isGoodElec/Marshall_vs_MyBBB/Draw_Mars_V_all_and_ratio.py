...
def compare_pair(phi_csv: str,
    ...
    if spec_error_col in df_spec.columns:
        # keep same-length vector; df_spec_small is same length as df_spec subset above
        df_spec_small["spec_error"] = df_spec[spec_error_col].to_numpy()
    else:
    ...
    merged["diff_phi_minus_spec"] = merged["phi_content_sum"] - merged["spec_content"]
    merged["ratio_phi_over_spec"] = merged["phi_content_sum"] / merged["spec_content"].replace(0, np.nan)
    ...
    # =============================================================================
    # 4) PLOTS: one figure per xQ2, subplots per z, content + ratio
    # =============================================================================
    ...
    for xq2 in xq2_values:
        ...
        fig.suptitle(f"xQ2 bin {xq2}: pT² dependence — phi-sum vs spec + ratio(phi/spec)", y=0.995)
        ...
        for idx, z in enumerate(z_values):
            ...
            r = sub["ratio_phi_over_spec"].to_numpy()

            # left axis: contents
            mphi = np.isfinite(y_phi)
            if mphi.any():
                (hphi,) = ax.plot(x[mphi], y_phi[mphi], marker="o", linestyle="-", label="phi-sum")
                if handle_phi is None:
                    handle_phi = hphi

            mspec = np.isfinite(y_spec)
            if mspec.any():
                (hspec,) = ax.plot(x[mspec], y_spec[mspec], marker="x", linestyle="--", label="spec")
            ...
            mrat = np.isfinite(r)
            if mrat.any():
                (hrat,) = ax2.plot(x[mrat], r[mrat], marker="s", linestyle=":", label="phi/spec", color = "#2ca02c" )
                if handle_ratio is None:
                    handle_ratio = hrat

                # --- enforce minimum ratio y-range: [0.95, 1.05] ---
                rmin = float(np.nanmin(r[mrat]))
                rmax = float(np.nanmax(r[mrat]))

                # If data are inside [0.95,1.05], keep at least that range.
                # If data exceed it, expand to include them.
                ylo = min(0.95, rmin)
                yhi = max(1.05, rmax)

                # Avoid zero-height axis if all ratios identical
            ...
            # label ratio y-axis only on rightmost column to reduce clutter
            if (idx % ncols) == (ncols - 1):
                ax2.set_ylabel("phi/spec")
            else:
                ax2.set_ylabel("")
        ...
        if handle_ratio is not None:
            handles.append(handle_ratio); labels.append("phi/spec")
        if handles:
            fig.legend(handles, labels, loc="upper right", bbox_to_anchor=(0.995, 0.98), fontsize=10)

        fig.tight_layout(rect=[0, 0, 0.98, 0.97])

        png = os.path.join(outdir, f"xq2_{xq2:02d}_pt2_by_z_subplots_with_ratio.png")
        pdf = os.path.join(outdir, f"xq2_{xq2:02d}_pt2_by_z_subplots_with_ratio.pdf")
...
def main():
    ap = argparse.ArgumentParser(
        description="Compare (phi-binned hist CSV) summed over phi vs (no-phi initial spectrum CSV). "
                    "Makes one figure per xQ2 with subplots per z showing pT2 dependence + ratio."
    )
