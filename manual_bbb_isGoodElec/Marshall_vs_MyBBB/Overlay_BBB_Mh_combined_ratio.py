...
def main():
    ...
    if args.plots_dir:
        ...
        for xq2, df_x in merged.groupby("xq2_bin"):
            ...
            # One combined figure:
            # for each logical cell -> overlay on top, ratio below
            W_PER_COL = 5.5
            H_OVERLAY = 3.0
            H_RATIO   = 1.2

            fig = plt.figure(
                figsize=(W_PER_COL * ncols, (H_OVERLAY + H_RATIO) * nrows),
                layout="constrained",
            )
            ...
            gs = fig.add_gridspec(
                2 * nrows,
                ncols,
                height_ratios=np.array([H_OVERLAY, H_RATIO] * nrows, dtype=float),
            )
            ...
            for i, z in enumerate(z_bins):
                ...
                axR = fig.add_subplot(gs[2 * row + 1, col], sharex=ax)
                ...
                if col == 0:
                    ax.set_ylabel("Mh (no abs. norm.)")
                    axR.set_ylabel("ratio")

                if row == nrows - 1:
                    axR.set_xlabel(r"$p_T^2$ center")
            ...
            for j in range(len(z_bins), total_slots):
                row, col = divmod(j, ncols)
                ax_unused = fig.add_subplot(gs[2 * row, col])
                axR_unused = fig.add_subplot(gs[2 * row + 1, col])
                ax_unused.axis("off")
                axR_unused.axis("off")

            fig.suptitle(
                f"Overlay + ratio vs pT2 "
                f"(xq2_bin={xq2}, scale_content={cs:.3g}, scale_acc={ca:.3g}, "
                f"DRAW_COEF={DRAW_COEF:.6g}; m3 no coef)"
            )
\
        ...
        print(f"Wrote combined overlay+ratio PNG plots into: {args.plots_dir}")
