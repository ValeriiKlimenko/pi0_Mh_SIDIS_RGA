...
def gather_points_per_xq2(tree: ROOT.TTree):
    ...
    for i in range(nentries):
        ...
        data.setdefault(ix, {}).setdefault((zb, pb), []).append((phi_deg, y, yerr))
...
def gather_sim_points_per_xq2(files, tree_name="h22_fit"):
    ...
    for i in range(nentries):
        ...
        cellmap = tmp.setdefault(ix, {}).setdefault((z_bin, pt2_bin), {})
...
def draw_one_xq2(ix, hmap, params, has_full_fit,
    ...
    for iz in range(1, N_ZBINS+1):
        for ipt in range(1, N_PT2_BINS_WITH_OVERFLOW+1):
            ...
            h   = hmap.get(ix, {}).get((iz, ipt), None)
            p   = params.get((ix, iz, ipt), None)
            ...
            if pts_data:
                ymax = max(ymax, max(y for (_, y, _) in pts_data))
            if pts_sim:
                ymax = max(ymax, max(y * sim_scale for (_, y, _) in pts_sim))
            if p and has_full_fit:
                p0, p1, p2 = p
                ymax = max(ymax, p0 + abs(p1) + abs(p2))
            if ymax <= 0:
                ymax = 1.0
            ...
            # Draw simulation graph (green, scaled)
            if pts_sim:
                gs = ROOT.TGraphErrors(len(pts_sim))
                for i,(x,y,yerr) in enumerate(pts_sim):
                    gs.SetPoint(i, x, y*sim_scale)
                    gs.SetPointError(i, 0.0, yerr*sim_scale)
                gs.SetMarkerStyle(21)
                gs.SetMarkerSize(0.7)
                gs.SetLineWidth(1)
                gs.SetLineColor(ROOT.kGreen+2)
                gs.SetMarkerColor(ROOT.kGreen+2)
                gs.Draw("PC SAME")
                graphs_sim.append(gs)


            # Draw data graph (black)
            if pts_data:
            ...
            # Draw fit curve (red) if we have full p0,p1,p2
            if p and has_full_fit:
                p0, p1, p2 = p
                f = ROOT.TF1(
                    f"f_fit_ix{ix}_z{iz}_pt{ipt}",
    ...
    dummy_hist.SetLineColor(ROOT.kBlue+1)
    dummy_hist.SetMarkerColor(ROOT.kBlue+1)
    dummy_hist.SetMarkerStyle(24)

    dummy_data = ROOT.TH1F("dummy_data", "", 1, 0, 1)
    dummy_data.SetLineColor(ROOT.kBlack)
    dummy_data.SetMarkerColor(ROOT.kBlack)
    dummy_data.SetMarkerStyle(20)

    dummy_sim = ROOT.TH1F("dummy_sim", "", 1, 0, 1)
    dummy_sim.SetLineColor(ROOT.kGreen+2)
    dummy_sim.SetMarkerColor(ROOT.kGreen+2)
    dummy_sim.SetMarkerStyle(21)

    leg.AddEntry(dummy_hist, "unfolded φ-slices", "lep")
    leg.AddEntry(dummy_data, "data (cover)", "lep")
    leg.AddEntry(dummy_sim,  "simulation (scaled)", "lep")

    if has_full_fit:
        dummy_fit = ROOT.TF1("dummy_fit", "0", 0, 1)
        dummy_fit.SetLineColor(ROOT.kRed)
        dummy_fit.SetLineWidth(2)
        leg.AddEntry(dummy_fit, "fit p_{0}+p_{1}cos#phi+p_{2}cos2#phi", "l")
