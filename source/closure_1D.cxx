...
//------- QA: simple 1D QA + migration writeout + MC closure test -------------
static void ResponseQA(const ResponseBundle& pack, const char* out="qa_response.root") {
  // pull what we can from the response
  ...
  // ------------------------------------------------------------------
  // MC closure test:
  //   - use Hmeasured (MC reco) as pseudo-data
  //   - unfold with the trained response
  //   - compare to Htruth (MC truth)
  // ------------------------------------------------------------------
  if (pack.resp && hm && ht) {
    // You can tune this number of iterations if you like
    const int nIterClosure = 5;

    std::cout << "Running MC closure test with Bayes, " << nIterClosure
              << " iterations...\n";

    RooUnfoldBayes cl(pack.resp.get(), hm, nIterClosure);
    cl.SetVerbose(0);

    TH1* h_unf_raw = dynamic_cast<TH1*>(cl.Hunfold(RooUnfold::kErrors));
    if (!h_unf_raw) {
      std::cerr << "Closure: Hunfold returned null.\n";
    } else {
      f.cd();

      // Unfolded spectrum for closure
      TH1* hU = (TH1*)h_unf_raw->Clone("h_unfold_closure");
      hU->SetTitle(Form("MC closure: unfold(Hmeasured) (Bayes, %d iter);global_bin;entries",
                        nIterClosure));
      hU->Write();

      // Ratio: unfolded / truth
      TH1* hRatio = (TH1*)ht->Clone("h_closure_ratio");
      hRatio->Reset("ICES");
      const int nb = ht->GetNbinsX();
      for (int i=1; i<=nb; ++i) {
        const double t = ht->GetBinContent(i);
        const double u = hU->GetBinContent(i);
        const double r = (t != 0.0) ? (u / t) : 0.0;
        hRatio->SetBinContent(i, r);
        hRatio->SetBinError(i, 0.0); // you can propagate errors if you wish
      }
      hRatio->SetTitle("MC closure ratio: unfolded / truth;global_bin;ratio");
      hRatio->Write();

      // Optional: global chi2 / ndf using unfolded errors
      double chi2 = 0.0;
      int ndf = 0;
      for (int i=1; i<=nb; ++i) {
        const double t = ht->GetBinContent(i);
        const double u = hU->GetBinContent(i);
        const double e = hU->GetBinError(i); // only unfolded error
        if (e <= 0.0) continue;
        const double d = u - t;
        chi2 += (d*d)/(e*e);
        ++ndf;
      }
      std::cout << "Closure chi2/ndf = " << chi2 << " / " << ndf
                << " = " << (ndf>0 ? chi2/ndf : 0.0) << std::endl;
    }
  } else {
    std::cerr << "Closure: missing response or MC histos; skipped.\n";
  }
...
// ------------------- MC closure test (Bayes, 1D global bin) ------------------
static int RunClosureTestBayes(const ResponseBundle& pack,
                               int         nIter    = 5,
                               const char* out_file = "closure_test_bayes.root")
{
  // Need trained response + MC truth + MC measured
  if (!pack.resp || !pack.h_true || !pack.h_meas) {
    std::cerr << "Closure ERROR: response / MC histos not built.\n";
    return 1;
  }

  TH1* hm = (TH1*)pack.h_meas.get();  // MC reco, used as pseudo-data
  TH1* ht = (TH1*)pack.h_true.get();  // MC truth

  if (!hm || !ht) {
    std::cerr << "Closure ERROR: missing h_meas or h_true.\n";
    return 2;
  }

  // Optional: sanity check binning
  if (!axes_identical(*hm, *ht)) {
    std::cerr << "Closure WARNING: h_meas and h_true have different binning.\n";
  }

  std::cout << "Running MC closure test (Bayes, " << nIter << " iterations)...\n";

  // --- Unfold MC reco -> should reproduce MC truth if everything is consistent
  RooUnfoldBayes u(pack.resp.get(), hm, nIter);
  u.SetVerbose(0);

  TH1* h_unfold_raw = dynamic_cast<TH1*>(u.Hunfold(RooUnfold::kErrors));
  if (!h_unfold_raw) {
    std::cerr << "Closure ERROR: Hunfold returned null.\n";
    return 3;
  }

  // Clone to own histogram living beyond RooUnfold object
  TH1* h_unfold = (TH1*)h_unfold_raw->Clone("h_unfold_closure");
  h_unfold->SetTitle(Form("MC closure: unfold(Hmeas) (Bayes, %d iter);global_bin;entries",
                          nIter));

  // --- Build ratio and difference histograms vs truth
  TH1* h_ratio = (TH1*)ht->Clone("h_closure_ratio");
  h_ratio->Reset("ICES");
  h_ratio->SetTitle("MC closure: unfolded / truth;global_bin;ratio");

  TH1* h_diff = (TH1*)ht->Clone("h_closure_diff");
  h_diff->Reset("ICES");
  h_diff->SetTitle("MC closure: unfolded - truth;global_bin;difference");

  const int nb = ht->GetNbinsX();
  for (int i=1; i<=nb; ++i) {
    const double t  = ht->GetBinContent(i);
    const double u  = h_unfold->GetBinContent(i);
    const double eT = ht->GetBinError(i);
    const double eU = h_unfold->GetBinError(i);

    // difference
    const double d = u - t;
    h_diff->SetBinContent(i, d);
    // propagate errors in quadrature
    const double e2 = eT*eT + eU*eU;
    h_diff->SetBinError(i, (e2>0.0 ? std::sqrt(e2) : 0.0));

    // ratio (unfolded / truth)
    double r = 0.0;
    if (t != 0.0) r = u / t;
    h_ratio->SetBinContent(i, r);
    // you can set an error if you like; for now, keep 0
    h_ratio->SetBinError(i, 0.0);
  }

  // --- Simple chi2/ndf using both truth + unfolded errors
  double chi2 = 0.0;
  int    ndf  = 0;
  for (int i=1; i<=nb; ++i) {
    const double t  = ht->GetBinContent(i);
  ...
  if (fout.IsZombie()) {
    std::cerr << "Closure ERROR: cannot create " << out_file << "\n";
    return 4;
  }

  // Save the MC input histos as well (for convenience)
  {
    TH1* h_meas_clone = (TH1*)hm->Clone("h_meas_mc");
    WriteHist1D(h_meas_clone, "h_meas_mc", fout);
  }
  {
    TH1* h_true_clone = (TH1*)ht->Clone("h_true_mc");
    WriteHist1D(h_true_clone, "h_true_mc", fout);
  }

  WriteHist1D(h_unfold, "h_unfold_closure", fout);
  WriteHist1D(h_ratio,  "h_closure_ratio",  fout);
  WriteHist1D(h_diff,   "h_closure_diff",   fout);

  fout.Write();
  fout.Close();

  std::cout << "Closure test wrote: " << out_file << "\n";
  return 0;
}
                   ...
                   bool write_response_snapshot = false)
{
  ...
  // Run up to 4 Bayes iterations and write ONE ROOT FILE PER ITERATION.
  // Filenames: unfold_bayes_iter01.root, unfold_bayes_iter02.root, ...
  int maxIter = nIter;
  if (maxIter <= 0) maxIter = 1;   // at least 1
  if (maxIter > 4)  maxIter = 4;   // cap at 4 as requested

  return unfold_bayes_save_each_iter(
           pack,
           maxIter,
           /*out_base      =*/ "unfold_bayes",
           /*put_all_in_one=*/ false,   // <-- separate file per iteration
           /*make_pngs     =*/ false);
...
int onepass_closure_bayes(int  nIter     = 5,
                          const char* out_file = "closure_test_bayes.root",
                          bool write_response_snapshot = false)
{
  std::cout << "Building response in memory for closure test..." << std::endl;

  ResponseBundle pack;
  if (!build_response_in_memory(pack,
                                /*include_fakes=*/false,
                                write_response_snapshot,
                                /*optional_resp_file=*/"response_out.root")) {
    std::cerr << "ERROR: build_response_in_memory failed.\n";
    return 10;
  }

  return RunClosureTestBayes(pack, nIter, out_file);
}