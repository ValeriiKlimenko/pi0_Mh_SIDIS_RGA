                                     ...
                                     const char* optional_resp_file = "response_out.root")
{
  ...
  // ---- Truth-only Misses ----
  if (df_miss_ptr) {
    TH2D h_truth_like("h_truth_like","Truth-like;zpt2phit_8x8x9;bin_xBQ2_Valerii", nZ,z_lo,z_hi, nX,x_lo,x_hi);
    h_truth_like.SetDirectory(nullptr);
    df_miss_ptr->Foreach(
      [&](int z_truth_like, int x_truth_like){
        const double zt = (double)z_truth_like, xt = (double)x_truth_like;
        if (in_range(zt,z_lo,z_hi) && in_range(xt,x_lo,x_hi)) h_truth_like.Fill(zt, xt, 1.0);
      },
    ...
    for (int iz=1; iz<=h_truth_minus_rec.GetXaxis()->GetNbins(); ++iz) {
      ...
      for (int ix=1; ix<=h_truth_minus_rec.GetYaxis()->GetNbins(); ++ix) {
        ...
        if (wmiss > 0.0) {
          ...
          // --- AUDIT: miss (truth-only); store integer-like indices from bin centers
          a_zgen = static_cast<int>(zc);
          a_xgen = static_cast<int>(xc);
          a_zrec = -1; a_xrec = -1;
          a_w = wmiss; a_kind = 2; tfill->Fill();
...
static void ResponseQA(const ResponseBundle& pack, const char* out="qa_response.root") {
  ...
  // ---- Column/row normalised migration (needs M only) ----
  TH2* M_col = nullptr;
  TH2* M_row = nullptr;
  if (M) {
    M_col = (TH2*) M->Clone("h_mig_colNorm");
    M_row = (TH2*) M->Clone("h_mig_rowNorm");
    const int nx = M->GetNbinsX(), ny = M->GetNbinsY();

    for (int jy=1; jy<=ny; ++jy) {
      double s=0; for (int ix=1; ix<=nx; ++ix) s += M->GetBinContent(ix,jy);
      if (s>0) for (int ix=1; ix<=nx; ++ix)
        M_col->SetBinContent(ix,jy, M->GetBinContent(ix,jy)/s);
      else     for (int ix=1; ix<=nx; ++ix)
        M_col->SetBinContent(ix,jy, 0.0);
    }
    for (int ix=1; ix<=nx; ++ix) {
      double s=0; for (int jy=1; jy<=ny; ++jy) s += M->GetBinContent(ix,jy);
      if (s>0) for (int jy=1; jy<=ny; ++jy)
        M_row->SetBinContent(ix,jy, M->GetBinContent(ix,jy)/s);
      else     for (int jy=1; jy<=ny; ++jy)
        M_row->SetBinContent(ix,jy, 0.0);
    }
    write_if(M_col);
    write_if(M_row);
  }
