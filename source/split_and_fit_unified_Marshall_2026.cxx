...
// If nphi_eff == 1, this effectively "ignores phi" (phibin will always be 1).
static inline void DecodeZpt2Phi(int cy, int& zbin, int& pt2bin, int& phibin, int nphi_eff) {
    if (cy <= 0) { zbin = pt2bin = phibin = 0; return; }

    const int zpt2 = ((cy - 1) / nphi_eff) + 1;   // 1..(N_Zbins * kNpt2_with_overflow)
    phibin         = ((cy - 1) % nphi_eff) + 1;   // 1..nphi_eff (==1 if no-phi)
...
static void split_and_fit_unified(const std::string& path_to_root, Logic logic, int isNoPhi = 0,
                                  int png_every = -1) {
    ...
    if (isData) {
        ...
        for (auto& s : slices) {
            ...
            int phibin = 0;
            int nphi_eff = (isNoPhi == 1) ? 1 : kNphi;
            DecodeZpt2Phi(out_zpt2phi, out_zbin, out_pt2bin, phibin, nphi_eff);
